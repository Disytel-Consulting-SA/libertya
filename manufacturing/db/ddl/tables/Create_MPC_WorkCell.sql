 ...
 * All Rights Reserved.
 *************************************************************************
 * Title:	 Create MPC Work Cell Table and Elements
 * Description:
 *	Creates and updates all Work Cell elements as desired in the Compiere 
...
DELETE FROM COMPIERE.AD_TABLE   WHERE UPPER(TABLENAME)  = 'MPC_WORKCELL'
DELETE FROM COMPIERE.AD_ELEMENT WHERE UPPER(COLUMNNAME) = 'MPC_WORKCELL_ID'

DROP TABLE MPC_WorkCell;

-- -----------------------------------------------------------------------------
-- Create the database table definition
-- -----------------------------------------------------------------------------

CREATE TABLE MPC_WorkCell(
    ...
    CONSTRAINT MPC_WorkCell_Key PRIMARY KEY (MPC_WorkCell_ID)
    USING INDEX
        PCTFREE 10
        INITRANS 2
        MAXTRANS 255
    ...
    CONSTRAINT RefMPC_Facility2176 FOREIGN KEY (MPC_Facility_ID)
    REFERENCES MPC_Facility(MPC_Facility_ID),
    CONSTRAINT RefS_Resource2179 FOREIGN KEY (S_Resource_ID)
...
COMMENT ON TABLE MPC_WorkCell IS 'Work Cells consist of one or more Work Centers used to produce ''like'' products with similar routings. '
;
-- -----------------------------------------------------------------------------
...
UPDATE COMPIERE.AD_TABLE   
   SET ENTITYTYPE = 'A', 
             NAME = 'MPC_WorkCell',    
        TABLENAME = 'MPC_WorkCell',
      ACCESSLEVEL = '1'                -- Organization
  WHERE UPPER(TABLENAME) = 'MPC_WORKCELL';
