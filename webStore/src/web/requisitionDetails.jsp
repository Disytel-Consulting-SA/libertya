					...
					<tr>
					  ...
					  <fmt:formatNumber value='${order.grandTotal}' type="currency" currencySymbol=""/> &euro;</strong></td>
						...
						<c:choose>
							...
							<c:when test='${order.docStatus=="VO"}'>Anulada</c:when>
							<c:when test='${order.docStatus=="CO"}'>Completada</c:when>
							<c:otherwise>Procesada</c:otherwise>
						</c:choose>
					  </td>
					</tr>
				</table>
