...
<c:if test='${empty webUser || !webUser.loggedIn}'>
	<c:redirect url='loginServlet?ForwardTo=requisitionInfo.jsp'/>
</c:if>
		...
		<div id="wrap">
			...
			<div id="center">
				<c:if test='${not empty webOrder}'>
					<h1>Su solicitud ha sido procesado correctamente</h1>
					...
					<!--[if IE]> </div> <![endif]-->
					<p>Para ver los detalles del mismo, haga <a href="requisitionDetails.jsp?C_Order_ID=<c:out value='${webOrder.c_Order_ID}'/>">click aqu&iacute;</a>.</p>
					<p>Para volver al listado completo de solicitudes, haga <a href="requisitions.jsp">click aqu&iacute;</a>.</p>
				</c:if>
				<c:if test='${empty webOrder}'>
					<h1>Ha ocurrido un error mientras se procesaba su pedido</h1>
					<p class="warning">No existe la solicitud a procesar. Trate de introducir de nuevo su solicitud.</p>
				</c:if>			
			</div>
