...
public class VCreateFromShipment extends VCreateFrom {
    ...
    protected boolean dynInit() throws Exception {
        ...
        // Remitos de Salida:
        if (isSOTrx()) {
            // Por el momento solo se permiten crear remitos de salida a partir de pedidos
            // ya que el remito de salida debe estar asociado si o si a un pedido.
        	invoiceLabel.setVisible(false);
        	invoiceField.setVisible(false);
       	
        	// Se setea el pedido que esté configurado en el encabezado del remito
        	if (getInOut().getC_Order_ID() > 0) {
        		orderField.setValue(getInOut().getC_Order_ID());
        		orderChanged(getInOut().getC_Order_ID());
        	}
        	
        	// Si el remito ya contiene líneas entonces no puede editarse el pedido
        	// debido a que las líneas del remito están si o si asociadas con las
    ...
    protected void initBPDetails( int C_BPartner_ID ) {
        ...
        StringBuffer display = new StringBuffer( "i.DocumentNo||' - '||" ).append( DB.TO_CHAR( "DateInvoiced",DisplayType.Date,Env.getAD_Language( Env.getCtx()))).append( "|| ' - ' ||" ).append( DB.TO_CHAR( "GrandTotal",DisplayType.Amount,Env.getAD_Language( Env.getCtx())));
    ...
    private void loadInvoice(int C_Invoice_ID) {
        ...
        sql.append("SELECT ")                                                                           // Entered UOM
           ...
           .append("GROUP BY l.QtyInvoiced, l.QtyEntered/l.QtyInvoiced, l.C_UOM_ID, l.M_Product_ID, p.Name, l.C_InvoiceLine_ID, l.Line, l.C_OrderLine_ID, l.Description ")
        ...
        for (SourceEntity sourceEntity : getSelectedSourceEntities()) {
            ...
            if( !iol.save()) {
               throw new CreateFromSaveException(
            		   "@InOutLineSaveError@ (# " + docLine.lineNo + "):<br>" + 
            ...
            // Create Invoice Line Link
                ...
                if (!il.save()) {
                    throw new CreateFromSaveException(
                 		   "@InvoiceLineSaveError@ (# " + il.getLine() + "):<br>" + 
                 		   CLogger.retrieveErrorAsString()
                    );
                }
            }
	...
	protected String getOrderFilter() {
    	...
    	// entra mercadería, con lo cual no se deben validar pendientes de entrega).
     	filter
     		...
     		.append(  "OR ")
     		.append(   "(C_Order.IsSOTrx='Y' AND POSITION('+' IN '").append(getInOut().getMovementType()).append("') > 0)")
     	    .append(  "OR ")	
     	    .append(   "(C_Order.IsSOTrx='N' AND POSITION('-' IN '").append(getInOut().getMovementType()).append("') > 0)")
     		.append(")");
	...
	protected boolean beforeAddOrderLine(OrderLine orderLine) {
		...
		String sql = 
			"SELECT COUNT(*) FROM M_InOutLine WHERE M_InOut_ID = ? AND C_OrderLine_ID = ?";
		Long count = (Long)DB.getSQLObject(null, sql, 
				new Object[] { getInOut().getM_InOut_ID(), orderLine.orderLineID }
		);
