...
public class ProductLinesSalesDataSource extends QueryDataSource {
	...
	protected String getQuery() {
		...
		sql.append("select pl.m_product_lines_id, pl.value as product_line_value, pl.name as product_line_name, sum(il.costprice * il.qtyentered) as cost, sum((il.priceentered * il.qtyentered) - il.documentdiscountamt) as sales " +
					 ...
					 "where il.ad_org_id = ? AND pl.isactive = 'Y' AND i.docstatus IN ('CO','CL') AND dt.docbasetype = '"+MDocType.DOCBASETYPE_ARInvoice+"' AND dt.doctypekey NOT IN ('RCI') AND date_trunc('day',i.dateinvoiced) >= date_trunc('day',?::date) AND date_trunc('day',i.dateinvoiced) <= date_trunc('day',?::date) ");
