...
public class SalesByHourDataSource extends QueryDataSource {
	...
	protected String getQuery() {
		String sql = "select hour, time_ini, time_end, total::numeric(22,9), sales::numeric(22,9), cant_tickets::integer, (CASE WHEN total = 0 THEN 0 ELSE sales/total END)::numeric(11,2) as part_total " +
					 ...
					 "						where i.ad_org_id = ? AND i.docstatus IN ('CO','CL') AND dt.docbasetype IN ('"+MDocType.DOCBASETYPE_ARInvoice+"','"+MDocType.DOCBASETYPE_ARCreditMemo +"') AND dt.doctypekey NOT IN ('RCI','RCR') AND date_trunc('day',i.dateinvoiced) >= date_trunc('day',?::date) AND date_trunc('day',i.dateinvoiced) <= date_trunc('day',?::date) ) as i on i.hour = h.hour) as a, " +
