...
public class ExportArciba extends SvrProcess {
	...
	private String getSqlArcibaInvoiceOriginal() {
	...
	private String getSqlArcibaCreditNoteOriginal(boolean imputed) {
	...
	/**
	 * Metodos para la obtencion de la consulta SQL basado en ajustes hechos para Siasa Oct 25
	 * 
	 * @param rs
	 * @return
	 * @throws IOException
	...
	private String getSqlArcibaInvoice() {
		...
		// Percepciones
		sqlReal.append(" SELECT * "
				+ "FROM ( "
				+ "SELECT "
				+ "	'2' AS TIPO_DE_OPERACION, " // Percepcion
				+ "	lpad(it.arcibanormcode::TEXT, 3, '0') AS CODIGO_DE_NORMA, "
				+ "	To_char(i.dateinvoiced, 'dd/MM/yyyy') AS FECHA_DE_RETEN_PERC, "
				+ "	(CASE "
				+ "		WHEN (d.docbasetype = 'ARI'AND doctypekey ILIKE 'CI%') THEN (CASE "
				+ "			WHEN ( SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_FACTURA_ARCIBA') IS NULL THEN '01' "
				+ "			ELSE ( SELECT value FROM AD_Preference 	WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_FACTURA_ARCIBA') "
				+ "		END) "
				+ "		WHEN (d.docbasetype = 'ARI' AND doctypekey ILIKE 'CDN%') THEN (CASE "
				+ "			WHEN (SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_NOTA_DEBITO_ARCIBA') IS NULL THEN '09' "
				+ "			ELSE ( SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_NOTA_DEBITO_ARCIBA') "
				+ "		END) "
				+ "		ELSE (CASE WHEN (SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_OTRO_ARCIBA') IS NULL THEN '09' "
				+ "			ELSE ( SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'TIPO_DE_COMPROBANTE_OTRO_ARCIBA') "
				+ "		END) "
				+ "	END) AS TIPO_DE_COMPROBANTE, "
				+ "	lc.letra AS LETRA_DE_COMPROBANTE, "
				+ "	lpad(substring(i.documentno FROM 2 FOR 13), '16', '0') AS NRO_DE_COMPROBANTE, "
				+ "	To_char(i.dateinvoiced, 'dd/MM/yyyy') AS FECHA_DE_COMPROBANTE, "
				+ "	REPLACE(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(i.grandtotal,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE i.GrandTotal END)::TEXT, 16, '0'), '.', ',') AS MONTO, "
				+ "	rpad('', 16, ' ') AS NRO_DE_CERTIFICADO_PROPIO, " // se debe completar con blancos NO con ceros
				+ "	bp.taxidtype AS TIPO_DOCUMENTO, " // Revisar esto... 3=CUIT, 2=CUIL, 1=CDI
				+ "	lpad(trim(bp.taxid), '11', '0') AS NRO_DE_DOCUMENTO, "
				+ "	COALESCE(bp.IIBBType, '4') AS SITUACION_IIBB, " // Revisar esto... 1: Local 2: Convenio Multilateral 4: No inscripto 5: Reg.Simplificado
				+ "	COALESCE(lpad(REPLACE(bp.iibb, '-', ''), '11', '0'), '00000000000') AS NRO_INCRIPCION_IIBB, "
				+ "	(CASE "
				+ "		WHEN (ci.codigo = 2) THEN '1' "
				+ "		WHEN (ci.codigo = 3) THEN '2' "
				+ "		WHEN (ci.codigo = 4) THEN '3' "
				+ "		WHEN (ci.codigo = 5) THEN '4' "
				+ "		WHEN (ci.codigo = 7) THEN '0' "
				+ "		ELSE '5' "
				+ "	END) AS SITUACION_IVA, " // 1 - Responsable Inscripto, 3 - Exento, 4 - Monotributo
				+ "	rpad(bp.name, '30', ' ') AS RAZON_SOCIAL_DEL_RETENIDO, "
				+ "	(CASE "
				+ "		WHEN (lc.letra = 'A' "
				+ "			OR lc.letra = 'M') THEN REPLACE(lpad(COALESCE((SELECT sum((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(it2.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE it2.taxamt END)) "
				+ "												FROM c_invoicetax it2 "
				+ "												INNER JOIN c_tax t ON it2.c_tax_id = t.c_tax_id " 
				+ "                                             INNER JOIN c_taxcategory ct ON ct.c_taxcategory_id = t.c_taxcategory_id "
				+ "												WHERE it2.c_invoice_id = i.c_invoice_id AND t.ispercepcion = 'Y'"
				+ "                                             AND ct.Name <> 'PERCEPCIONES'  "
				+"                                              ),0.00)::TEXT, 16, '0'), '.', ',') "
				+ "		ELSE REPLACE(lpad(( it.taxamt + (SELECT SUM((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(ita.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE ita.taxamt END)) AS IVA "
				+ "										FROM C_InvoiceTax ita "
				+ "										LEFT JOIN C_Tax ta ON (ta.C_Tax_ID = ita.C_Tax_ID) WHERE (ta.IsPercepcion = 'N') AND ita.C_Invoice_ID = i.C_Invoice_ID) )::TEXT, 16, '0'), '.', ',') "
				+ "	END) AS OTROS_CONCEPTOS, "
				+ "	(CASE "
				+ "		WHEN (lc.letra = 'A' "
				+ "			OR lc.letra = 'M') THEN REPLACE(lpad((SELECT SUM((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(ita.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE ita.taxamt END)) AS IVA "
				+ "												FROM C_InvoiceTax ita "
				+ "												LEFT JOIN C_Tax ta ON (ta.C_Tax_ID = ita.C_Tax_ID) WHERE (ta.IsPercepcion = 'N') AND ita.C_Invoice_ID = i.C_Invoice_ID)::TEXT, 16, '0'), '.', ',') "
				+ "		ELSE '0000000000000,00' "
				+ "	END) AS IVA, "
				+ "	REPLACE(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(i.netamount,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE i.netamount END)::TEXT, 16, '0'), '.', ',') AS MONTO_SUJETO_A_PERCEP_RETEN, "
				+ "	lpad(substring(it.rate::TEXT FROM 1 FOR POSITION('.' IN it.rate::TEXT) - 1), 2, '0') || ',' || rpad(substring(it.rate::TEXT FROM POSITION('.' IN it.rate::TEXT) + 1 FOR 5), 2, '0') AS ALICUOTA, "
				+ "	REPLACE(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(it.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE it.taxamt END)::TEXT, 16, '0'), '.', ',') AS RET_PERCEP_PRACTICADAS, "
				+ "	REPLACE(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(it.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE it.taxamt END)::TEXT, 16, '0'), '.', ',') AS TOTAL_MONTO_RETENIDO, "
				+ " 'Percepcion' AS tipo "
				+ "FROM "
				+ "	C_Invoice i "
				+ "LEFT JOIN C_InvoiceTax it ON (i.C_Invoice_ID = it.C_Invoice_ID) "
				+ "LEFT JOIN C_Tax t ON (t.C_Tax_ID = it.C_Tax_ID) "
				+ "LEFT JOIN C_Letra_Comprobante lc ON (i.C_Letra_Comprobante_ID = lc.C_Letra_Comprobante_ID) "
				+ "LEFT JOIN C_DocType d ON (GREATEST(i.C_DocType_ID, i.C_DocTypeTarget_ID) = d.C_DocType_ID) "
				...
				+ "	AND (i.docstatus = 'CO' OR i.docstatus = 'CL' OR i.docstatus = 'VO' OR i.docstatus = 'RE') "
				+ "	AND (t.C_Region_ID::TEXT =( SELECT value FROM AD_Preference WHERE ATTRIBUTE = 'C_Region_ID_FACTURA_ARCIBA')) "
				+ "	AND ((d.docbasetype = 'ARI'	AND doctypekey ILIKE 'CI%') OR (d.docbasetype = 'ARI' AND doctypekey ILIKE 'CDN%')) ");
		...
		// dREHER Retenciones
		String valueRetIIBBCaba = MPreference.GetCustomPreferenceValue("Value_EsquemaRetencionIIBBCaba", Env.getAD_Client_ID(getCtx()));
		if(Util.isEmpty(valueRetIIBBCaba, true))
			valueRetIIBBCaba = "RETIIBBCABA_EMIT";
				
		sqlReal.append(" UNION  "
				+ "SELECT "
				+ "	'1' AS TIPO_DE_OPERACION, " // Retencion
				+ "	'029' AS CODIGO_DE_NORMA, " // Verificar estos codigos
				+ "	to_char(i.dateinvoiced, 'dd/MM/yyyy') AS FECHA_DE_RETEN_PERC, "
				+ "	'03' AS TIPO_DE_COMPROBANTE, "   // OP
				+ "	COALESCE(lc.letra, ' ') AS LETRA_DE_COMPROBANTE, " // para las OP la letra va en blanco
				+ "	lpad(substring(ah.documentno FROM 3 FOR 13), '16', '0') AS NRO_DE_COMPROBANTE, "
				+ "	To_char(i.dateinvoiced, 'dd/MM/yyyy') AS FECHA_DE_COMPROBANTE, "
				+ "	REPLACE(lpad((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(ri.grandtotal,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE ri.grandtotal END)::TEXT, 16, '0'), '.', ',') AS MONTO, "
				+ "	lpad(substring(i.documentno FROM 3 FOR 13), '16', '0') AS NRO_DE_CERTIFICADO_PROPIO, "
				+ "	bp.taxidtype AS TIPO_DOCUMENTO, " // Revisar esto... 3=CUIT, 2=CUIL, 1=CDI
				+ "	lpad(trim(bp.taxid), '11', '0') AS NRO_DE_DOCUMENTO, "
				+ "	COALESCE(bp.IIBBType, '4') AS SITUACION_IIBB, "
				+ "	COALESCE(lpad(REPLACE(bp.iibb, '-', ''), '11', '0'), '00000000000') AS NRO_INCRIPCION_IIBB, "
				+ "	(CASE "
				+ "		WHEN (ci.codigo = 2) THEN '1' "
				+ "		WHEN (ci.codigo = 3) THEN '2' "
				...
				+ "		WHEN (ci.codigo = 7) THEN '0' "
				+ "		ELSE '5' "
				+ "	END) AS SITUACION_IVA, "
				+ "	rpad(bp.name, '30', ' ') AS RAZON_SOCIAL_DEL_RETENIDO, "
				+ "	(SELECT REPLACE(trim(to_char(COALESCE(sum((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(it.taxamt,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE it.taxamt END)),0),'0000000000000.00')), '.', ',') "
				+ "	FROM c_invoicetax it "
				+ "	INNER JOIN c_invoice i ON i.c_invoice_id = it.c_invoice_id "
				+ "	INNER JOIN c_tax t ON t.c_tax_id = it.c_tax_id "
				+ "	WHERE "
				+ "		t.ispercepcion = 'Y' " // verificar el tilde percepcion en el impuesto, debe ser retencion ?
				+ "		AND i.c_invoice_id IN (  "
				+ "		SELECT al.c_invoice_id "
				+ "		FROM c_allocationline al "
				+ "		JOIN c_allocationhdr ah ON ah.c_allocationhdr_id = al.c_allocationhdr_id "
				+ "		WHERE ah.c_allocationhdr_id = ri.c_allocationhdr_id)) AS OTROS_CONCEPTOS, "
				+ "	(SELECT REPLACE(trim(to_char(COALESCE(sum((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(it.taxamt,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE it.taxamt END)),0),'0000000000000.00')), '.', ',') "
				+ "	FROM c_invoicetax it "
				+ "	INNER JOIN c_invoice i ON i.c_invoice_id = it.c_invoice_id "
				+ "	INNER JOIN c_tax t ON t.c_tax_id = it.c_tax_id "
				+ "	WHERE t.ispercepcion = 'N' "
				+ "		AND i.c_invoice_id IN ( "
				+ "		SELECT al.c_invoice_id "
				+ "		FROM c_allocationline al "
				+ "		JOIN c_allocationhdr ah ON ah.c_allocationhdr_id = al.c_allocationhdr_id "
				+ "		WHERE ah.c_allocationhdr_id = ri.c_allocationhdr_id)) AS IVA, "
				+ "	REPLACE(trim(to_char((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(ri.pago_actual_amt,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE ri.pago_actual_amt END),'0000000000000.00')), '.', ',') AS MONTO_SUJETO_A_PERCEP_RETEN, "
				+ "	REPLACE(trim(to_char(ri.retencion_percent,'00.00')), '.', ',') AS ALICUOTA, "
				+ "	REPLACE(trim(to_char((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(ri.amt_retenc,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE ri.amt_retenc END),'0000000000000.00')), '.', ',') AS RET_PERCEP_PRACTICADAS, "
				+ "	REPLACE(trim(to_char((CASE WHEN ri.C_Currency_ID<>118 THEN currencyConvert(ri.amt_retenc,ri.C_Currency_ID,118,ri.DateInvoiced,null,ri.AD_Client_ID,ri.AD_Org_ID) ELSE ri.amt_retenc END),'0000000000000.00')), '.', ',') AS TOTAL_MONTO_RETENIDO, "
				+ " 'Retencion' AS tipo "
				+ "FROM m_retencion_invoice_v ri "
				...
				+ "JOIN c_doctype dt ON dt.c_doctype_id = (GREATEST(i.c_doctype_id, i.c_doctypetarget_id)) "
				...
				+ "LEFT JOIN c_categoria_iva ci ON ci.c_categoria_iva_id = bp.c_categoria_iva_id "
				+ "WHERE "
				+ "	ri.ad_client_id = " + getAD_Client_ID() + " "
				+ "	AND rs.value = '" + valueRetIIBBCaba + "' ");  // dREHER E-IIBB_CABA'
		
		if (ad_org_id > 0){
			sqlReal.append(" AND (ri.AD_Org_ID = "+ad_org_id+")");
		}
		
		if (date_from != null){
			sqlReal.append(" AND (date_trunc('day',ri.DateInvoiced) >= '"+date_from+"')");
		}
		
		if (date_to != null){
			sqlReal.append(" AND (date_trunc('day',ri.DateInvoiced) <= '"+date_to+"')");
		}

		sqlReal.append(" ) q  ORDER BY q.FECHA_DE_RETEN_PERC;");
		
		System.out.println("SQL debitos= " + sqlReal.toString());
	...
	private String getSqlArcibaCreditNote(boolean imputed) {
		...
		/**
		 * Por cuestiones de tasa de cambio y redondeos, los valores no dan correctamente, mejor calcular el neto de otra moneda
		 * en funcion del impuesto
		 * dREHER sep 24
		if(imputed) {
			sqlReal.append("replace(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(i.netamount,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE i.netamount END)::text,16,'0'),'.',',') AS MONTO_NOTA_DE_CREDITO, ");
		} else {			
			sqlReal.append("replace(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(i.grandtotal,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE i.GrandTotal END)::text,16,'0'),'.',',') AS MONTO_NOTA_DE_CREDITO, ");
		}
		*/
		
		// 5 si es imputada -> Neto de la NC, si a cuenta -> Total de la NC
		if(imputed) {
			sqlReal.append("replace(lpad((CASE WHEN i.C_Currency_ID<>118 THEN ROUND(currencyConvert(it.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID)/(it.rate/100),2) ELSE ROUND(i.netamount,2) END)::text,16,'0'),'.',',') AS MONTO_NOTA_DE_CREDITO, ");			
		} else {			
			sqlReal.append("replace(lpad((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(i.grandtotal,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE i.GrandTotal END)::text,16,'0'),'.',',') AS MONTO_NOTA_DE_CREDITO, ");
		}
		
		// 6 - Nro Certificado Propio - Texto(16) - 16 - Texto en Blanco
		sqlReal.append("rpad('',16,' ') AS NRO_DE_CERTIFICADO_PROPIO, ");
		// 7 - Tipo Comprobante Origen - Número(2) - 2
		sqlReal.append("(SELECT (CASE WHEN (doc.docbasetype = 'ARI' AND doc.doctypekey ILIKE 'CI%') THEN (CASE WHEN (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_FACTURA_ARCIBA') IS NULL THEN '01' "
				+ "ELSE (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_FACTURA_ARCIBA') END) "
				+ "WHEN (doc.docbasetype = 'ARI' AND doc.doctypekey ILIKE 'CND%') "
				+ "THEN (CASE WHEN (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_NOTA_DEBITO_ARCIBA') IS NULL THEN '02' "
				+ "ELSE (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_NOTA_DEBITO_ARCIBA') END) "
				+ "ELSE (CASE WHEN (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_OTRO_ARCIBA') IS NULL THEN '09' "
				+ "ELSE (SELECT value FROM AD_Preference WHERE attribute = 'TIPO_DE_COMPROBANTE_OTRO_ARCIBA') END) END) "
				+ "FROM C_Invoice inv "
				+ "LEFT JOIN C_DocType doc ON (inv.C_DocType_ID = doc.C_DocType_ID) WHERE inv.C_Invoice_ID = i.C_Invoice_Orig_ID) AS TIPO_DE_COMPROBANTE_ORIGEN, ");
		// 7 - Letra de Comprobante - Texto(1) - 1
		sqlReal.append("(SELECT lc.letra FROM C_Invoice inv LEFT JOIN C_Letra_Comprobante lec ON (inv.C_Letra_Comprobante_ID = lec.C_Letra_Comprobante_ID) WHERE inv.C_Invoice_ID = i.C_Invoice_Orig_ID) AS LETRA_DE_COMPROBANTE, ");
		// 9 - Número de Comprobante - Texto(16) - 16
		sqlReal.append("(SELECT lpad(substring(inv.documentno from 2 for 13),'16','0') FROM C_Invoice inv WHERE inv.C_Invoice_ID = i.C_Invoice_Orig_ID) AS NRO_DE_COMPROBANTE, ");
		// 10 - Número de Documento del Retenido- Número(11) - 11
		sqlReal.append("lpad(trim(bp.taxid),'11','0') AS NRO_DE_DOCUMENTO, ");
		// 11 - Código de Norma - Número(3) - 3
		sqlReal.append("lpad((SELECT itax.arcibanormcode::text FROM C_InvoiceTax itax WHERE itax.C_Invoice_ID = i.C_Invoice_Orig_ID AND itax.arcibanormcode IS NOT NULL),3,'0') AS CODIGO_DE_NORMA, ");
		// 12 - Fecha de Reten/Percep - Fecha(10) - 10
		sqlReal.append("To_char((SELECT inv.dateinvoiced FROM C_Invoice inv WHERE inv.C_Invoice_ID = i.C_Invoice_Orig_ID),'dd/MM/yyyy') AS FECHA_DE_RETEN_PERC, ");
		
		// 13 - Ret/Percep Practicadas - Numero(13,2) - 16
		/**
		 * Por cuestiones de calculo de tasas de cambio y redondeos termina no dando correctamente la retencion/percepcion
		 * dREHER
		 
		sqlReal.append("replace(lpad(((CASE WHEN i.C_Currency_ID<>118 THEN (currencyConvert(i.netamount,\r\n"
				+ "				i.C_Currency_ID,\r\n"
				+ "				118,\r\n"
				+ "				i.DateInvoiced,\r\n"
				+ "				null,\r\n"
				+ "				i.AD_Client_ID,\r\n"
				+ "				i.AD_Org_ID) * it.rate / 100) ELSE it.taxamt END)::numeric(16,2))::text,16,'0'),'.',',') AS RET_PERCEP_PRACTICADAS, ");		
		 */
		
		sqlReal.append("replace(lpad(((CASE WHEN i.C_Currency_ID<>118 THEN currencyConvert(it.taxamt,i.C_Currency_ID,118,i.DateInvoiced,null,i.AD_Client_ID,i.AD_Org_ID) ELSE it.taxamt END)::numeric(16,2))::text,16,'0'),'.',',') AS RET_PERCEP_PRACTICADAS, ");
		
		// 14 - Alícuota - Numero(2,2) - 5
		sqlReal.append("lpad(substring(it.rate::text from 1 for position('.' in it.rate::text) - 1),2,'0')  || ',' || rpad(substring(it.rate::text from position('.' in it.rate::text) + 1 for 5),2,'0') AS ALICUOTA, ");			
		// 99 - Fecha de Comprobante Origen - Utilizada para filtrar		
		sqlReal.append("(SELECT inv.dateinvoiced FROM C_Invoice inv WHERE inv.C_Invoice_ID = i.C_Invoice_Orig_ID) AS FECHA_COMPROBANTE_ORIGEN ");
		sqlReal.append("FROM C_Invoice i ");
		...
		sqlReal.append("LEFT JOIN C_Tax t ON (t.C_Tax_ID = it.C_Tax_ID) ");
		sqlReal.append("LEFT JOIN C_Letra_Comprobante lc ON (i.C_Letra_Comprobante_ID = lc.C_Letra_Comprobante_ID) "); 
		sqlReal.append("LEFT JOIN C_DocType d ON (GREATEST(i.C_DocType_ID, i.C_DocTypeTarget_ID) = d.C_DocType_ID) ");
		sqlReal.append("LEFT JOIN C_BPartner bp ON (i.C_BPartner_ID = bp.C_BPartner_ID) ");
		sqlReal.append("LEFT JOIN C_Categoria_IVA ci ON (ci.C_Categoria_IVA_ID = bp.C_Categoria_IVA_ID) ");

		sqlReal.append("WHERE (i.isActive = 'Y') AND (i.AD_CLIENT_ID = "+getAD_Client_ID()+") AND (d.isfiscaldocument = 'Y') AND (t.ispercepcion = 'Y') ");
		sqlReal.append("AND (i.docstatus = 'CO' OR i.docstatus = 'CL' OR i.docstatus = 'VO' OR i.docstatus = 'RE') ");
		sqlReal.append("AND (t.C_Region_ID::text =(SELECT value FROM AD_Preference WHERE attribute = 'C_Region_ID_FACTURA_ARCIBA')) ");
		sqlReal.append("AND (d.docbasetype = 'ARC' AND doctypekey ILIKE 'CCN%') ");
		...
		System.out.println("SQL creditos= " + sqlReal.toString());
		
		return sqlReal.toString();
	}

	
	private int putRsInFileInvoiceOriginal(ResultSet rs) throws IOException, SQLException{
	...
	private int putRsInFileInvoice(ResultSet rs) throws IOException, SQLException{
		...
		while(rs.next())
		{
			...
			System.out.println("Generando: " + rs.getString("Tipo"));
			
			s=new StringBuffer();
			
			s.append(rs.getString(1)); // tipo, en este caso retencion
			s.append(rs.getString(2)); // codigo de registro
			s.append(rs.getString(3)); // fecha de retencion
			s.append(rs.getString(4)); // tipo de operacion
			...
			s.append(rs.getString(6)); // nro de comprobante
			s.append(rs.getString(7)); // fecha del comprobante
			
		// Monto sujeto a retencion/percecion	
			BigDecimal montoSujetoARetencion = new BigDecimal(rs.getString(18).replace(",", "."));
			
		// Total del comprobante	
			String montoTmp = rs.getString(8);
			if(montoTmp==null || montoTmp.isEmpty())
				montoTmp = "0.00";
			
			BigDecimal montoTotal = new BigDecimal(montoTmp.replace(",", "."));
		
		// Otros conceptos	
			montoTmp = rs.getString(16);
			if(montoTmp==null || montoTmp.isEmpty())
				montoTmp = "0.00";
			
			BigDecimal otrosConceptos = new BigDecimal(montoTmp.replace(",", "."));
		
		// IVA	
			montoTmp = rs.getString(17);
			if(montoTmp==null || montoTmp.isEmpty())
				montoTmp = "0.00";

			BigDecimal importeIva = new BigDecimal(montoTmp.replace(",", "."));
			
			
			BigDecimal montoAux = montoTotal.subtract(otrosConceptos).subtract(importeIva);
			
			// Total del comprobante
			// s.append(rs.getString(8)); // <-- original
			
			// dREHER calcular el total del comprobante contemplando los valores intermedios
			
			/** para retencion OP NO se carga IVA */
			if(rs.getString(1).equals("1")) 
				montoTotal = montoSujetoARetencion.add(otrosConceptos);
			else
				montoTotal = montoSujetoARetencion.add(importeIva).add(otrosConceptos);
			
			// Control para casos donde el monto total esta mal, montosujeto a retencion es cero
			/*
			if(rs.getString(1).equals("1") && montoSujetoARetencion.compareTo(Env.ZERO) == 0) {
				
				BigDecimal retenido = new BigDecimal(rs.getString(21).replace(",", "."));
				BigDecimal tasa = new BigDecimal(rs.getString(19).replace(",", "."));
		...
		//	if(rs.getString(1).equals("1")) {
			...
			// Total monto retenido
				montoTmp = rs.getString(21);
				if(montoTmp==null || montoTmp.isEmpty())
					montoTmp = "0.00";
			
				BigDecimal retenido = new BigDecimal(montoTmp.replace(",", "."));
				
			// Alicuota
				montoTmp = rs.getString(19);
				if(montoTmp==null || montoTmp.isEmpty())
					montoTmp = "0.00";
				
				BigDecimal tasa = new BigDecimal(montoTmp.replace(",", "."));
				
				// La tasa no puede ser cero, normalmente cuando se carga la retencion en la OP
				// no se carga este dato y provoca una division por cero
				// dREHER Oct 25
		...
		//	}
			...
			String smonto = montoTotal.toString();
			s.append(("0000000000000000".substring(smonto.length()) + smonto).replace(".", ","));
			
			s.append(rs.getString(9)); // nro interno de retencion
			...
			s.append(rs.getString(11)); // nro de cuit/cuil/cdi
			 ...
			 * 1: Local, 2: Convenio Multilateral, 4: No inscripto, 5: Reg.Simplificado 
			 */
			s.append(rs.getString(12)); // situacion iibb
			
			// verificar si corresponde o no nro de inscripcion o cuit
			if(rs.getString(12).equals("4"))
				s.append("00000000000");
			else if(rs.getString(12).equals("1")) 
				s.append(rs.getString(13)); // nro de inscripcion
			else
				s.append(rs.getString(11)); // nro de cuit/cuil/cdi
			
			s.append(rs.getString(14)); // situacion iva 
			s.append(rs.getString(15)); // razon social
			
			s.append(rs.getString(16)); // otros impuestos
			
			/** para retencion OP NO se carga IVA */
			if(rs.getString(1).equals("1")) 
				s.append("0000000000000,00");
			else
				s.append(rs.getString(17)); // iva
			...
			*/
			...
			montoTmp = rs.getString(8);
			if(montoTmp==null || montoTmp.isEmpty())
				montoTmp = "0.00";
			
			montoTotal = new BigDecimal(montoTmp.replace(",", "."));
			...
			*/
			...
			// dREHER tomar el monto sujeto a retencion desde el calculo
			String monto = montoSujetoARetencion.toString();
			s.append(("0000000000000000".substring(monto.length()) + monto).replace(".", ","));
			
			// -------------------------------------------------------------------------------------------------------------------
			// tasa de retencion
			s.append(("00000".substring(tasa.toString().length()) + tasa.toString()).replace(".", ","));
			// s.append(rs.getString(19)); 
			
			
			s.append(rs.getString(20)); // retenido
			s.append(rs.getString(21)); // retenido
