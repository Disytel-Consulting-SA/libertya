...
public class ImportGLJournal extends SvrProcess {
    ...
    protected String doIt() throws java.lang.Exception {
		...
		/* OXP FT no tiene M_CentroCosto
		//actualizado - 26-08-08 - Horacio Alvarez

		//	Set Org from Centro Costo
		sql = new StringBuffer ("UPDATE I_GLJournal i "
        ...
        */
        ...
        sql = new StringBuffer( "UPDATE I_GLJournal o " + "SET I_IsImported='E', I_ErrorMsg=I_ErrorMsg||'ERR=Invalid Doc Org, '" + "WHERE (AD_OrgDoc_ID IS NULL OR AD_OrgDoc_ID=0" + " OR EXISTS (SELECT * FROM AD_Org oo WHERE o.AD_Org_ID=oo.AD_Org_ID AND (oo.IsSummary='Y' OR oo.IsActive='N')))" + " AND I_IsImported<>'Y'" ).append( clientCheck );
        no = DB.executeUpdate( sql.toString());
...
//            + " WHERE c.Name like 'Standard' AND i.AD_Client_ID=c.AD_Client_ID) " 
        ...
        sql = new StringBuffer( "UPDATE I_GLJournal i " + "SET CurrencyRate=(SELECT r.MultiplyRate FROM C_Conversion_Rate r, C_AcctSchema s" + " WHERE s.C_AcctSchema_ID=i.C_AcctSchema_ID AND s.AD_Client_ID=i.AD_Client_ID" + " AND r.C_Currency_ID=i.C_Currency_ID AND r.C_Currency_ID_TO=s.C_Currency_ID" + " AND r.AD_Client_ID=i.AD_Client_ID" + " AND r.C_ConversionType_ID=i.C_ConversionType_ID" + " AND i.DateAcct BETWEEN r.ValidFrom AND r.ValidTo AND ROWNUM=1"
...
//			+ " AND i.C_AcctSchema_ID=ase.C_AcctSchema_ID AND i.AD_Client_ID=ev.AD_Client_ID AND ROWNUM=1) "
            + " WHERE ev.Value=i.AccountValue AND ev.IsSummary='N'" + " AND i.C_AcctSchema_ID=ase.C_AcctSchema_ID AND i.AD_Client_ID=ev.AD_Client_ID AND ROWNUM=1) " 
            + "WHERE Account_ID IS NULL AND AccountValue IS NOT NULL" + " AND (C_ValidCombination_ID IS NULL OR C_ValidCombination_ID=0) AND I_IsImported<>'Y'" ).append( clientCheck );
        no = DB.executeUpdate( sql.toString());
...
/*		sql = new StringBuffer ("UPDATE I_GLJournal "
			...
			+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString ());
		log.debug ("doIt - Set 0 Source Dr=" + no);
		sql = new StringBuffer ("UPDATE I_GLJournal "
			+ "SET AmtSourceCr = 0 "
			+ "WHERE AmtSourceCr IS NULL"
			+ " AND I_IsImported<>'Y'").append (clientCheck);
		no = DB.executeUpdate (sql.toString ());
		log.debug ("doIt - Set 0 Source Cr=" + no);
		sql = new StringBuffer ("UPDATE I_GLJournal i "
		...
		no = DB.executeUpdate (sql.toString ());
		if (no != 0)
			log.warn ("doIt - Zero Source Balance=" + no);*/
...
//					log.warn("doIt - Balance Source=" + source  + ", Acct=" + acct);
...
//					addLog (0, null, source, "@AmtSourceDr@ - @AmtSourceCr@");
        ...
        sql = new StringBuffer( "SELECT * FROM I_GLJournal " + "WHERE I_IsImported='N'" ).append( clientCheck ).append( " ORDER BY BatchDocumentNo, JournalDocumentNo," + "C_AcctSchema_ID, PostingType, C_DocType_ID, GL_Category_ID, C_Currency_ID, TRUNC(DateAcct), Line, I_GLJournal_ID" );
        ...
        try {
            ...
            while( rs.next()) {
                ...
                if( (journal == null) ||!JournalDocumentNo.equals( impJournalDocumentNo ) || (journal.getC_AcctSchema_ID() != imp.getC_AcctSchema_ID()) || (journal.getC_DocType_ID() != imp.getC_DocType_ID()) || (journal.getGL_Category_ID() != imp.getGL_Category_ID()) ||!journal.getPostingType().equals( imp.getPostingType()) || (journal.getC_Currency_ID() != imp.getC_Currency_ID()) ||!impDateAcct.equals( DateAcct )) {
...
//			AGREGADO - HORACIO - 21/08/08
			...
			for(int i = 0;i < batches.size();i++)
			{
				...
				if( ! batch.completeIt().equals(MJournalBatch.DOCSTATUS_Completed))
				{
					...
					translation = Msg.parseTranslation(Env.getCtx(), batch.getProcessMsg());
					stmt.setString(1,"No se pudo completar el asiento: "+translation);
