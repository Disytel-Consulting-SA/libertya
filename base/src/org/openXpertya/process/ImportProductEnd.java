...
public class ImportProductEnd extends SvrProcess {
    ...
    protected String doIt() throws java.lang.Exception {
        ...
        String[] numFieldsPO = new String[] {
            "C_UOM_ID","C_Currency_ID","RoyaltyAmt","Order_Min","Order_Pack","CostPerOrder","DeliveryTime_Promised"
        };
        ...
        try {
           ...
           // PreparedStatement pstmt_updateProduct = conn.prepareStatement( "UPDATE M_PRODUCT " + "SET Value=aux.value,Name=aux.name,Description=aux.description,DocumentNote=aux.documentnote,Help," + "UPC,SKU,C_UOM_ID,M_Product_Category_ID,Classification,ProductType," + "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet," + "Discontinued,DiscontinuedBy,Updated,UpdatedBy,M_Product_Family_ID) from " + 
           // 		"(SELECT Value,Name,Description,DocumentNote,Help," + "UPC,SKU,C_UOM_ID,M_Product_Category_ID,Classification,ProductType," + "Volume,Weight,ShelfWidth,ShelfHeight,ShelfDepth,UnitsPerPallet," + "Discontinued,DiscontinuedBy,CURRENT_TIMESTAMP,UpdatedBy,M_Product_Category_ID" + " FROM I_Product WHERE I_Product_ID=?) as aux " + "WHERE M_Product_ID=?" );
        ...
        +" WHERE M_Product_ID=?");
           ...
           // PreparedStatement pstmt_updateProductPO = conn.prepareStatement( "UPDATE M_Product_PO " + "SET (IsCurrentVendor,C_UOM_ID,C_Currency_ID,UPC," + "PriceList,PricePO,RoyaltyAmt,PriceEffective," + "VendorProductNo,VendorCategory,Manufacturer," + "Discontinued,DiscontinuedBy,Order_Min,Order_Pack," + "CostPerOrder,DeliveryTime_Promised,Updated,UpdatedBy)= " + "(SELECT 'Y',C_UOM_ID,C_Currency_ID,UPC," + "PriceList,PricePO,RoyaltyAmt,PriceEffective," + "VendorProductNo,VendorCategory,Manufacturer," + "Discontinued,DiscontinuedBy,Order_Min,Order_Pack," + "CostPerOrder,DeliveryTime_Promised,CURRENT_TIMESTAMP,UpdatedBy" + " FROM I_Product" + " WHERE I_Product_ID=?) " + "WHERE M_Product_ID=? AND C_BPartner_ID=?" );
            PreparedStatement pstmt_updateProductPO = conn.prepareStatement("UPDATE M_Product_PO " + "SET IsCurrentVendor='Y'"+
        		    ...
        		    ",DiscontinuedBy=aux1.DiscontinuedBy"+
...
/*
 *  @(#)ImportProductEnd.java   02.07.07
 * 
 *  Fin del fichero ImportProductEnd.java
 *  
 *  Versión 2.2
 *
 */
