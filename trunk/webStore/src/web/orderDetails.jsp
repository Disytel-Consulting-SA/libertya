...
<html>
	...
	<body onLoad="initClock()">
		...
		<div id="wrap">
			...
			<div id="center">
				<h1>Detalles del pedido <c:out value='${order.documentNo}'/></h1>
					  ...
					  <fmt:formatNumber value='${order.grandTotal}' type="currency" currencySymbol=""/> &euro;</strong></td>
						...
						<c:choose>
							<c:when test='${order.docStatus=="IP"}'>
								<input name="Void" id="Void" value="Anular" onClick="window.top.location.replace('<c:out value='https://${ctx.context}/'/>orderServlet?C_Order_ID=<c:out value='${order.c_Order_ID}'/>&DocAction=VO');" type="button" class="button">
							</c:when>
