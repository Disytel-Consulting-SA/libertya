		...
		<div id="wrap">
			...
			<div id="center">
				<c:if test='${not empty webOrder}'>
					<h1>Gracias por realizar su pedido con nosotros</h1>
					...
					<table class="tablelist">
						...
						<tr> 
							<td><a href="orderDetails.jsp?C_Order_ID=<c:out value='${webOrder.c_Order_ID}'/>">
					...
					<h3>Detalles del pedido realizado</h3>
					<table class="tablelist">
						<tr> 
						...
						<tr> 
							...
							<td> 
								<c:choose>
									...
									<c:when test='${webOrder.deliveryRule=="O"}'>S&oacute;lo si pedido completo</c:when>
									...
									<c:otherwise>Otras políticas de env&iacute;o</c:otherwise>
								</c:choose>
						  </td>
							<td>
								<c:choose>
							...
							<td> 
								<c:choose>
									<c:when test='${order.docStatus=="IP"}'>
										<input name="Void" id="Void" value="Anular el pedido" onClick="window.top.location.replace('<c:out value='https://${ctx.context}/'/>orderServlet?C_Order_ID=<c:out value='${webOrder.c_Order_ID}'/>&DocAction=VO');" type="button" class="button">
									</c:when>
					...
					<h2 align="center">Para ver los detalles del mismo, pulse sobre el n&uacute;mero de pedido.</h2>
				</c:if>
				<c:if test='${empty webOrder}'>
					<p class="warning">No existe el pedido a procesar. Trate de introducir de nuevo su pedido.</p>
				</c:if>			
			</div>
