 ...
 * 
 */
public class C_RemesaGenerate34 extends SvrProcess
{
	/**
	 * 	ExportData Constructor
	 */
	public C_RemesaGenerate34()
	{
		super();
		log.fine("C_RemesaGenerate34");
	}	//	ImportBPartner
	...
	{
		log.fine("En C_Remesa34, en doIt");
		try { 
		...
		{
			log.fine("C_RemesaGenerate34 - " + fnfe.toString());
		}
		catch (IOException e)
		{
		}
 ...
 *         *Copyright � ConSerTi
	*/
	private void cabeceraOrdenante()
	{
		...
		sql += " bac.AccountNo, rem.C_Remesa_ID, nor.subnorma, bac.iban, bac.oficina, bac.sucursal, bac.dc, bac.cc, org.name, loc.address1, loc.plaza FROM C_Remesa rem";
		...
		try
		{
			...
			while (rs.next())
			{
				...
				sb.append(rs.getString(1)); //Codigo Reg 2 pos---03
				sb.append(rs.getString(2)); //Codigo Dato 2 pos--56
				...
				if(cont<10){
					...
					log.fine("El registro tiene mas de 10 posiciones");
				}
				sb.append(aux); //NIF 10 pos
				...
				c.setTime(rs.getDate(5));//Fecha emisión ordenes
				if(c.get(Calendar.DAY_OF_MONTH)<10){			//Dia con 2 digitos
					sb.append("0"+c.get(Calendar.DAY_OF_MONTH));
				}else{					
					sb.append(c.get(Calendar.DAY_OF_MONTH));
				...
				sb.append(String.valueOf(c.get(Calendar.YEAR)).substring(2,4));//Año 2 digitos
				aux=rs.getString(6);
				sb.append(aux.substring(0, 3));//Nro entidad 4 dig
				sb.append(aux.substring(4,7));//Nro oficina 4 dig
				sb.append(aux.substring(10,aux.length()));//Cuenta corriente
				//sb.append(rs.getString(10)+rs.getString(11)+rs.getString(13));//Numero entidad,Nro oficina,Nro cuenta
				sb.append("1");//Detalle del cargo,1 apunte contable por beneficiario
				sb.append("   ");//Libre de 3 posiciones
				sb.append(aux.substring(8,9));//Digito control de la cuenta
				sb.append("       ");//Libre de 7 posiciones
				guardarLinea(sb.toString());
				
				int rm = rs.getInt(7);
				log.fine("En cabeceraOrdenante34, para las lineas de pagos:"+rm);
				Cabecera2(rs);
				Beneficiario(rm);
		...
		{ 
			log.saveError("C_RemesaGenerate34 - cabeceraOrdenante", e);
		}
	}
	
	private void Cabecera2(ResultSet rs)
	{		
		int cont=0;
		String aux="";
		try{
		...
		sb.append(aux);//Nombre del ordenante
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
		Cabecera3(rs);
				/////Empieza segundo registro
		}catch (Exception e)
		{ 
			log.saveError("C_RemesaGenerate34 - cabecera2", e);
		}		
			
		
	}
	
	private void Cabecera3(ResultSet rs)
	{		
		int cont=0;
		String aux="";
		try{
		...
		sb.append(aux); //NIF 10 pos
		sb.append("            ");//Libre de 12 pos
		sb.append("003");//Número de dato
		cont=rs.getString(15).length();
		aux=rs.getString(15);
		if(cont<36){
			while(aux.length()<36){
				aux=aux.concat(" ");
		...
		sb.append(aux);//Domicilio del ordenante
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
		Cabecera4(rs);
		}catch (Exception e)
		{ 
			log.saveError("C_RemesaGenerate34 - Cabecera3", e);
		}		
			
		
	}
	
	private void Cabecera4(ResultSet rs)
	{		
		int cont=0;
		String aux="";
		try{
		...
		sb.append(aux);//Plaza del ordenante
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
		
		}catch (Exception e)
		{ 
			log.saveError("C_RemesaGenerate34 - Cabecera4", e);
		}		
 ...
 *         *Copyright � ConSerTi
	*/
	private void Beneficiario(int ID)
	{
		...
		sql += " bak.accountno, ips.DueAmt, nor.Description, bp.duns, ips.duedate, clo.address1, clo.postal, reg.name, trl.name,rem.c_remesa_id, cou.countrycode,ips.c_invoicepayschedule_id, nor.gastos, nor.concepto_orden,clo.plaza FROM C_InvoicePaySchedule ips";
		...
		try
		{
			...
			while (rs.next())
			{
				...
				sb.append(aux);//Referencia beneficiario 12 pos
				...
				if(cont<12){		
					...
					log.fine("Error, la cadena es mayor de 12");
					return;
				} 
				...
				sb.append(aux.substring(4,7));//Nro oficina 4 dig
				sb.append(aux.substring(10,aux.length()));//Cuenta corriente
				sb.append(rs.getString(18));//gastos 1-Ordenante, 2-Beneficiario
				sb.append(rs.getString(19));//concepto_orden 1-Nómina, 8-Pensión, 9-Otros
				guardarLinea(sb.toString());
				sb.append("  ");//Libre de 2 pos
				sb.append(aux.substring(8,9));//Digito control 2 pos
				sb.append("       ");//Libre de 7 pos
				/////Empieza segundo registro
				Beneficiario2(rs);
			}
	...
	{		
		...
		sb.append(aux);//Nombre del beneficiario
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
		Beneficiario3(rs);
				/////Empieza segundo registro
	...
	{		
		...
		sb.append(aux);//Direccion del beneficiario
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
		Beneficiario4(rs);
				/////Empieza segundo registro
	...
	{		
		...
		sb.append(aux);//Aqui iriaaaaa plaza del beneficiario
		sb.append("       ");//Libre de 7 pos
		guardarLinea(sb.toString());
	...
	*/
	private void resumenOrdenante()
	{
		...
		try
		{
			...
			{
				StringBuffer sb = new StringBuffer("");
				sb.append(rs.getString(1));//Código Registro 2 pos//08
				sb.append(rs.getString(2));//Código Dato 2 pos//56
				cont=rs.getString(3).length();
				aux=rs.getString(3);
				if(aux.contains("-")){
					aux=aux.replace("-", "");
				...
				sb.append(aux);//Número de registros de dato 010
