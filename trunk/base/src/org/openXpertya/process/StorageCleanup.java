...
public class StorageCleanup extends SvrProcess {
    ...
    protected String doIt() throws Exception {
        ...
        sql = "SELECT * " + "FROM M_Storage s " + "WHERE AD_Client_ID = ?" + " AND QtyOnHand < 0"

        // Instance Attribute

        + " AND EXISTS (SELECT * FROM M_Product p" + " INNER JOIN M_AttributeSet mas ON (p.M_AttributeSet_ID=mas.M_AttributeSet_ID) " + "WHERE s.M_Product_ID=p.M_Product_ID AND mas.IsInstanceAttribute='Y')"
        ...
        + " AND EXISTS (SELECT * FROM M_Storage sw" + " INNER JOIN M_Locator swl ON (sw.M_Locator_ID=swl.M_Locator_ID), M_Locator sl " + "WHERE sw.QtyOnHand > 0" + " AND s.M_Product_ID=sw.M_Product_ID" + " AND s.M_Locator_ID=sl.M_Locator_ID" + " AND sl.M_Warehouse_ID=swl.M_Warehouse_ID)";
    ...
    private void eliminateReservation( MStorage target ) {
        ...
        if( (target.getQtyReserved().signum() != 0) || (target.getQtyOrdered().signum() != 0) ) {
            ...
            MStorage storage0     = MStorage.get( getCtx(),M_Locator_ID,target.getM_Product_ID(),0,get_TrxName());
            ...
            if( storage0 == null ) {
                ...
                if( M_Locator_ID != defaultLoc.getM_Locator_ID()) {
                    ...
                    storage0     = MStorage.get( getCtx(),M_Locator_ID,target.getM_Product_ID(),0,get_TrxName());
                }
            }
            ...
            if( storage0 != null ) {
                ...
                if( (reserved.signum() != 0) || (ordered.signum() != 0) ) {
                    if( MStorage.add( getCtx(),target.getM_Warehouse_ID(),target.getM_Locator_ID(),target.getM_Product_ID(),target.getM_AttributeSetInstance_ID(),target.getM_AttributeSetInstance_ID(),Env.ZERO,reserved.negate(),ordered.negate(),get_TrxName())) {
                        if( MStorage.add( getCtx(),storage0.getM_Warehouse_ID(),storage0.getM_Locator_ID(),storage0.getM_Product_ID(),storage0.getM_AttributeSetInstance_ID(),storage0.getM_AttributeSetInstance_ID(),Env.ZERO,reserved,ordered,get_TrxName())) {
    ...
    private MStorage[] getSources( int M_Product_ID,int M_Locator_ID ) {
        ...
        String    sql  = "SELECT * " + "FROM M_Storage s " + "WHERE QtyOnHand > 0" + " AND M_Product_ID=?"

        // Empty ASI

        + " AND (M_AttributeSetInstance_ID=0" + " OR EXISTS (SELECT * FROM M_AttributeSetInstance asi " + "WHERE s.M_AttributeSetInstance_ID=asi.M_AttributeSetInstance_ID" + " AND asi.Description IS NULL) )"
...
/*
 *  @(#)StorageCleanup.java   02.07.07
 * 
 *  Fin del fichero StorageCleanup.java
 *  
 *  Versión 2.2
 *
 */
