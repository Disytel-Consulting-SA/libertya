...
public class ProductLinesSalesDataSource extends QueryDataSource {
	...
	protected String getQuery() {
		...
		sql.append("select pl.m_product_lines_id, pl.value as product_line_value, pl.name as product_line_name, sum(costprice*qtyentered) as cost, sum((priceentered-documentdiscountamt)*qtyentered) as sales " +
					 ...
					 "where il.ad_org_id = ? AND pl.isactive = 'Y' AND date_trunc('day',i.dateinvoiced) >= date_trunc('day',?::date) AND date_trunc('day',i.dateinvoiced) <= date_trunc('day',?::date) ");
