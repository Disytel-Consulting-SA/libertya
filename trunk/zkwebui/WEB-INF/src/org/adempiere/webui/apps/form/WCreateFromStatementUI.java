...
public class WCreateFromStatementUI extends CreateFromStatement implements EventListener
{
	...
	public boolean dynInit() throws Exception
	{
		...
		lookup = MLookupFactory.get (Env.getCtx(), p_WindowNo, 0, M_Column.getColumnID(MPayment.Table_Name, "C_DocType_ID", null), DisplayType.TableDir);
		documentTypeField = new WTableDirEditor ("C_DocType_ID",false,false,true,lookup);
		documentTypeField.getComponent().addEventListener(Events.ON_CHANGE, this);
		
		lookup = MLookupFactory.get (Env.getCtx(), p_WindowNo, 0, M_Column.getColumnID(MPayment.Table_Name, "TenderType", null), DisplayType.List);
		tenderTypeField = new WTableDirEditor ("TenderType",false,false,true,lookup);
		...
		Timestamp date = Env.getContextAsDate(Env.getCtx(), p_WindowNo, "StatementDate");
