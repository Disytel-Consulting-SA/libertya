...
public class WLocationDialog extends Window implements EventListener
{
	...
	public WLocationDialog(String title, MLocation location)
	{
		...
		if (m_location.getCountry().isHasRegion()) {
			if (m_location.getCountry().get_Translation("RegionName") != null
					&& m_location.getCountry().get_Translation("RegionName").trim().length() > 0)
				lblRegion.setValue(m_location.getCountry().get_Translation("RegionName"));
			else
	...
	private void initLocation()
	{
		...
		if (	m_location.getC_Region_ID() > 0 && 
				((X_C_Region)M_Table.get(Env.getCtx(), X_C_Region.Table_Name).getPO(m_location.getC_Region_ID(), null)).getC_Country_ID() == country.getC_Country_ID()) 
		{
			setRegion();
		} else {
		...
		Env.setContext(Env.getCtx(), m_WindowNo, Env.TAB_INFO, "C_Country_ID", String.valueOf(country.getID()));
