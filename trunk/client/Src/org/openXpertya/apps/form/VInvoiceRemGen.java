...
//Fin Modificacion
    ...
    private void dynInit() {
       ...
       // miniTable.setColumnClass( 6,Timestamp.class,true,Msg.translate( Env.getCtx(),"DatePrinted" ));
        ...
        //fFrom.setValue( new Timestamp( System.currentTimeMillis()));
        
        fTo.setMandatory( true );
        //fTo.setValue( new Timestamp( System.currentTimeMillis()));
    ...
    private void executeQuery() {
        ...
        //StringBuffer sql = new StringBuffer( "SELECT c_invoice_id, dateacct, ispaid, totallines, grandtotal, ispayschedulevalid, dateprinted, dateinvoiced, isindispute FROM c_invoice WHERE ispayschedulevalid like 'Y' AND AND DueDate<(SELECT rem.ExecuteDate FROM C_Remesa rem WHERE rem.C_Remesa_ID=" + Remesa + ")" );
        StringBuffer sql = new StringBuffer( "SELECT ips.c_invoicepayschedule_id, inv.documentno, cb.name, duedate, ispaid, dueamt, grandtotal, ispayschedulevalid, dateprinted, dateinvoiced, isindispute FROM c_invoicepayschedule ips, c_invoice inv, c_bpartner cb WHERE ips.c_invoice_id=inv.c_invoice_id AND cb.c_bpartner_id=inv.c_bpartner_id AND ((DueDate<(SELECT rem.ExecuteDate FROM C_Remesa rem WHERE rem.C_Remesa_ID="+Remesa+") AND ispaid='N')OR aux='Y') AND ips.Processed='N' " +
        ...
        //Timestamp FromDate= fFrom.getTimestamp();
        Date FromDate = fFrom.getTimestamp();
       // log.fine("La fecha que saca con date es="+FromDate.toString().substring(0, FromDate.toString().indexOf(" ")));
        if(FromDate!=null){
        	log.fine("Entro aqui1");
        	sql.append(" AND ips.created > '"+FromDate.toString().substring(0, FromDate.toString().indexOf(" "))+"'");
        }
        //Timestamp ToDate= fTo.getTimestamp();
        Date ToDate = fTo.getTimestamp();
   
        if(ToDate!=null){
        	log.fine("Entro aqui2");
        	//sql.append(" AND ips.created < DATE("+ToDate.toString().substring(0, ToDate.toString().indexOf(" "))+")");
